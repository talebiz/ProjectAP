...
public class Collision {
    ...
    private static void checkMeleeAttack() {
        ...
        for (Enemy enemy : EntityData.getEnemies()) {
            if (enemy.isCanMeleeAttack()) {
                if (outOfCheck(enemy, xEpsilon, yEpsilon)) continue;
    ...
    private static boolean outOfCheck(Enemy enemy, double xEpsilon, double yEpsilon) {
        return Math.abs(enemy.getX() - xEpsilon) > 100 ||
                Math.abs(enemy.getY() - yEpsilon) > 100;
    }
    ...
    private static void enemiesCollision() {
        for (int i = 0; i < EntityData.getEnemies().size() - 1; i++) {
            for (int j = i + 1; j < EntityData.getEnemies().size(); j++) {
                ...
                if (outOfCheck(enemy1, enemy2.getX(), enemy2.getY())) continue;
    ...
    private static void shotAndEnemyCollision() {
        for (Shot shot : EntityData.getShots()) {
            if (shot.getKind() == KindOfShot.EPSILON_SHOT) {
                ...
                for (Enemy enemy : EntityData.getEnemies()) {
                    if (outOfCheck(enemy, xShot, yShot)) continue;
                    if (enemy.getLines().length == 0) {
